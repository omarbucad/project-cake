...
class Products extends CI_Controller{
	...
	public function get_category(){
		...
		if($this->db->trans_status() === FALSE){
			echo json_encode(["status" => false , "message" => "Something went wrong", "action" => "get_category"]);
		}else{
			if($result){
				...
				echo json_encode(["status" => true , "data" => $result, "action" => "get_category"]);
			}else{
				echo json_encode(["status" => false , "message" => "No Results...", "action" => "get_category"]);
			}
		}		
	}
	...
	public function get_products($category_id) {
		...
		$this->db->trans_complete();
		if($this->db->trans_status() === FALSE){
			echo json_encode(["status" => false , "message" => "Something went wrong", "action" => "get_products"]);
		}else{
			if($result){
				foreach($result as $key => $row){
				...
				echo json_encode(["status" => true , "data" => $result, "action" => "get_products"]);
			}else{
				echo json_encode(["status" => false , "message" => "No Results...", "action" => "get_products"]);
			}
		}
	}
	...
	public function get_products_by_name($product_name) {
		...
		if($price_book_id){
			...
			$this->db->trans_complete();
			if($this->db->trans_status() === FALSE){
				echo json_encode(["status" => false , "message" => "Something went wrong", "action" => "get_products_by_name"]);
			}else{
				if($result){
					foreach($result as $key => $row){
						$image = $this->db->where("primary_image" , 1)->where("product_id" , $row->product_id)->get("products_images")->row();
					...
					echo json_encode(["status" => true , "data" => $result, "action" => "get_products_by_name"]);
				}else{
					echo json_encode(["status" => false , "message" => "No Results...", "action" => "get_products_by_name"]);
				}
			}
		}else{
			echo json_encode(["status" => false , "message" => "Something went wrong", "action" => "get_products_by_name"]);
	...
	public function get_product_by_id($product_id) {
		...
		if($price_book_id){
			...
			$this->db->trans_complete();
			if($this->db->trans_status() === FALSE){
				echo json_encode(["status" => false , "message" => "Something went wrong", "action" => "get_product_by_id"]);
			}else{
				...
				if($result){
					...
					echo json_encode(["status" => true , "data" => $result, "action" => "get_product_by_id"]);
				}else{
					echo json_encode(["status" => false , "message" => "No Results...", "action" => "get_product_by_id"]);
				}
			}
		}else{
			echo json_encode(["status" => false , "message" => "Something went wrong", "action" => "get_product_by_id"]);
	...
	public function checkOut(){
		...
		if($this->post){
			...
			if ($this->db->trans_status() === FALSE){
	            echo json_encode(["status" => false , "message" => "Checkout Failed , Please Try Again Later", "action" => "checkout"]);
	        }else{
	        	echo json_encode(["status" => true , "order_number" => $order_number, "action" => "checkout"]);
	        }
		}
	}
	...
	public function get_order_list($customer_id){
		...
		$this->db->trans_complete();
		if($this->db->trans_status() === FALSE){
			echo json_encode(["status" => false , "message" => "Something went wrong", "action" => "get_order_list"]);
		}else{
			if($result){
				foreach($result as $key => $row){
	        	...
	        	echo json_encode(["status" => true , "data" => $result, "action" => "get_order_list"]);
	        }else{
	        	echo json_encode(["status" => false , "message" => "No Results...", "action" => "get_order_list"]);
	        }
	...
	public function get_order_information_by_id($order_number){
		...
		if($result){
			foreach($result->product_list as $key => $row){
				...
				echo json_encode(["status" => true  , "data" => $result, "action" => "get_order_information_by_id"]);
		}else{
			echo json_encode(["status" => false , "message" => "No Results...", "action" => "get_order_information_by_id"]);
		}
	}
	...
	public function currently_order($customer_id){
		...
		$this->db->trans_complete();
		if($this->db->trans_status() === FALSE){
			echo json_encode(["status" => false , "message" => "Something went wrong" , "data" => [], "action" => "currently_order"]);
		}else{			
	        if($result){
				foreach($result as $key => $row){
		            $result[$key]->total_price = custom_money_format($row->total_price);
	        	...
	        	echo json_encode(["status" => true , "data" => $result, "action" => "currently_order"]);
	        }else{
	        	echo json_encode(["status" => false , "message" => "No Results..." , "data" => [], "action" => "currently_order"]);
	        }
		}
	}
	...
	public function cancel_order(){
		if($this->post){
			$this->db->trans_start();
			$cancelled = $this->db->where("order_number" , $this->post->order_number)->update("customer_order" , ["status" => 0]);
			$this->db->trans_complete();
			if($this->db->trans_status() === FALSE){
				echo json_encode(["status" => false, "action" => "cancel_order"]);
			}else{
				echo json_encode(["status" => true, "action" => "cancel_order"]);	
			}
		}
	}
}
