...
class Driver extends CI_Controller{
        ...
        */
		...
		if($this->db->trans_status() === FALSE){
			...
			if($result){
				...
				echo json_encode(["status" => false, "message" => "No data available", "action" => "get_order_list"]);
			}		
		}
	...
	public function get_order_information($order_number){
		...
		if($result){
			...
			echo json_encode(["status" => true  , "data" => $result, "action" => "get_order_information"]);
		}else{
			echo json_encode(["status" => false , "message" => "No Results...", "action" => "get_order_information"]);	
		}
	}
	...
	public function get_items_images($order_number){
		...
		$this->db->trans_complete();
		if($this->db->trans_status() === FALSE){
			echo json_encode(["status" => false , "message" => "No Results...", "action" => "get_items_images"]);
		}else{
			...
			if($tmp){
				echo json_encode(["status" => true  , "data" => $tmp, "action" => "get_items_images"]);
			}else{
				echo json_encode(["status" => false , "message" => "No Results...", "action" => "get_items_images"]);	
			}
		}
	...
	public function save_signature(){
		if($this->post){
			if($file_path = $this->save_image($this->post->order_number)){
				...
				echo json_encode(["status"=> true, "message" => "Signature saved", "action" => "save_signature"]);
			}else{
				echo json_encode(["status"=> false, "message" => "Something went wrong.", "action" => "save_signtature"]);
			}
		}
	}
	...
	public function change_password(){
		if($this->post){
			...
			$updated = $this->db->where("user_id" , $user_id)->update("users" , [
				"password"	=> $password
			]);
			if($updated){
				echo json_encode(["status" => true, "message" => "Updated successfully", "action" => "change_password"]);
			}else{
				echo json_encode(["status" => false, "message" => "Something went wrong.", "action" => "change_password"]);
			}
	...
	public function save_token(){
		if($this->post){
			...
			echo json_encode(["status"=> true, "action" => "save_token"]);
		}
	}
	...
	public function driver_logout(){
		if($this->post){
			...
			echo json_encode(["status" => true, "action" => "driver_logout"]);
		}
	}
	...
	public function send_items_image(){
		if($this->post){
			...
			if($file){
				...
				$this->db->trans_complete();
				if($this->db->trans_status() === FALSE){
					echo json_encode(["status"=> false,	"message" => "Something went wrong", "action" => "send_items_image"
					]);
				}else{
					echo json_encode([
						"status" 		=> true,
						...
						"action" => "send_items_image"
					]);
				}
			}else{
				echo json_encode(["status" => false,"message" => "Failed to process image.", "action" => "send_items_image"	]);
			}
		}
	}

	public function remove_image_items($image_id){
		$update = $this->db->where("image_id" , $image_id)->update("customer_order_images" , ["deleted" => time()]);
		if($update){
			echo json_encode(["status" => true, "action" => "remove_image_items"]);
		}else{
			echo json_encode(["status" => false, "action" => "remove_image_items"]);
		}
	...
	public function start_driving(){
		if($this->post){
			$this->db->trans_start();
			$started = $this->db->where("order_number" , $this->post->order_number)->update("customer_order" , [
				"start_driving" => time()
			]);
			$this->db->trans_complete();

			if($this->db->trans_status() === FALSE){
				echo json_encode(["status" => false, "action" => "start_driving"]);
			}else{

				echo json_encode(["status" => true, "action" => "start_driving"]);
